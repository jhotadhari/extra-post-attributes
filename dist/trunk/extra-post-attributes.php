...
class Expa_Extra_post_attributes {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('expa_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('expa_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('expa_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('expa_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('expa_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'expa_version', self::VERSION );
		add_option( 'expa_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'expa_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-expa-update.php' );
			// new Expa_Update();
			// class Expa_Update()  is missing ??? !!!
		}
	}
...
function expa_extra_post_attributes_init() {
	return Expa_Extra_post_attributes::get_instance();
}
expa_extra_post_attributes_init();
